...
$authenticode = New-SelfSignedCertificate -Subject "Tykord Powershell CERTs" -CertStoreLocation Cert:\LocalMachine\My -Type CodeSigningCert
...
## Close the TrustedPublisher certificate store.
 ...
 Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Subject -eq "CN=Tykord Powershell CERTs"}
# Confirm if the self-signed Authenticode certificate exists in the computer's Root certificate store
 Get-ChildItem Cert:\LocalMachine\Root | Where-Object {$_.Subject -eq "CN=Tykord Powershell CERTs"}
# Confirm if the self-signed Authenticode certificate exists in the computer's Trusted Publishers certificate store
 Get-ChildItem Cert:\LocalMachine\TrustedPublisher | Where-Object {$_.Subject -eq "CN=Tykord Powershell CERTs"}
 # Get the code-signing certificate from the local computer's certificate store with the name *Tykord Powershell CERTs* and store it to the $codeCertificate variable.
$codeCertificate = Get-ChildItem Cert:\LocalMachine\My | Where-Object {$_.Subject -eq "CN=Tykord Powershell CERTs"}
...
$PS2Sign = Read-Host "Enter Path to .ps1 file to be signed"
Set-AuthenticodeSignature -FilePath $PS2Sign -Certificate $codeCertificate -TimeStampServer http://timestamp.digicert.com